...
0000000000400efc <phase_2>:
  ...
  400f05:	e8 52 05 00 00       	call   40145c <read_six_numbers> !# 寄存器和低位栈指向高位栈中的空间
  400f0a:	83 3c 24 01          	cmpl   $0x1,(%rsp) # 比较第一个数字
  400f0e:	74 20                	je     400f30 <phase_2+0x34>
  400f10:	e8 25 05 00 00       	call   40143a <explode_bomb>
  400f15:	eb 19                	jmp    400f30 <phase_2+0x34>
  400f17:	8b 43 fc             	mov    -0x4(%rbx),%eax !# 循环头 指向偏移的前一个数字
  400f1a:	01 c0                	add    %eax,%eax # 乘2
  400f1c:	39 03                	cmp    %eax,(%rbx) !# 前一个数乘2等于后一个数
  400f1e:	74 05                	je     400f25 <phase_2+0x29>
  400f20:	e8 15 05 00 00       	call   40143a <explode_bomb>
  400f25:	48 83 c3 04          	add    $0x4,%rbx !# 便宜位置
  400f29:	48 39 eb             	cmp    %rbp,%rbx !# 比较是否循环完栈，即bx是否指向bp
  400f2c:	75 e9                	jne    400f17 <phase_2+0x1b>
  400f2e:	eb 0c                	jmp    400f3c <phase_2+0x40>
  400f30:	48 8d 5c 24 04       	lea    0x4(%rsp),%rbx # 指向第二个数字
  400f35:	48 8d 6c 24 18       	lea    0x18(%rsp),%rbp # 指向最后1位数字+4
  400f3a:	eb db                	jmp    400f17 <phase_2+0x1b> !# 循环调用
...
0000000000401338 <strings_not_equal>:
  ...
  40133a:	55                   	push   %rbp
  40133b:	53                   	push   %rbx # 保存用到的寄存器的值
  40133c:	48 89 fb             	mov    %rdi,%rbx # 输入地址的字符串地址被存到rbx中
  40133f:	48 89 f5             	mov    %rsi,%rbp # 答案地址被存到rbp中
  401342:	e8 d4 ff ff ff       	call   40131b <string_length>
  401347:	41 89 c4             	mov    %eax,%r12d # 返回数值默认寄存再eax中
  40134a:	48 89 ef             	mov    %rbp,%rdi # 这里将答案地址放到di当中，此时唯一的输入（）地址是rbx
  40134d:	e8 c9 ff ff ff       	call   40131b <string_length>
  401352:	ba 01 00 00 00       	mov    $0x1,%edx # 预设表示不相等为 1
  401357:	41 39 c4             	cmp    %eax,%r12d # 比较地址串长度
  40135a:	75 3f                	jne    40139b <strings_not_equal+0x63> # 这里直接跳到139b，结束比较,反汇编与实际运行存在地址差
  40135c:	0f b6 03             	movzbl (%rbx),%eax
  40135f:	84 c0                	test   %al,%al # 检查输入字符串是否为空 test <=> and，不会把结果写回目标寄存器
  401361:	74 25                	je     401388 <strings_not_equal+0x50>
  401363:	3a 45 00             	cmp    0x0(%rbp),%al
  401366:	74 0a                	je     401372 <strings_not_equal+0x3a>
  401368:	eb 25                	jmp    40138f <strings_not_equal+0x57>
  40136a:	3a 45 00             	cmp    0x0(%rbp),%al
  40136d:	0f 1f 00             	nopl   (%rax) # 占用位置保持对其的空指令
  401370:	75 24                	jne    401396 <strings_not_equal+0x5e>
  401372:	48 83 c3 01          	add    $0x1,%rbx
  401376:	48 83 c5 01          	add    $0x1,%rbp
  40137a:	0f b6 03             	movzbl (%rbx),%eax
  40137d:	84 c0                	test   %al,%al # 检查是否为空，字符串是否结束
  40137f:	75 e9                	jne    40136a <strings_not_equal+0x32> #循环
  401381:	ba 00 00 00 00       	mov    $0x0,%edx # 更新标志寄存器
  401386:	eb 13                	jmp    40139b <strings_not_equal+0x63>
  401388:	ba 00 00 00 00       	mov    $0x0,%edx
  40138d:	eb 0c                	jmp    40139b <strings_not_equal+0x63> # 这里存在两种相等是因为有不同路径表示相等
  40138f:	ba 01 00 00 00       	mov    $0x1,%edx
  401394:	eb 05                	jmp    40139b <strings_not_equal+0x63>
  401396:	ba 01 00 00 00       	mov    $0x1,%edx
  40139b:	89 d0                	mov    %edx,%eax # 函数结尾的统一返回点，记录返回数据并恢复寄存器
  40139d:	5b                   	pop    %rbx
  40139e:	5d                   	pop    %rbp
...
000000000040145c <read_six_numbers>: !# si 预先存入 sp 寄存器的内容
  40145c:	48 83 ec 18          	sub    $0x18,%rsp
  401460:	48 89 f2             	mov    %rsi,%rdx !# 第1个参数 数字的
  401463:	48 8d 4e 04          	lea    0x4(%rsi),%rcx !# lea取有效地址把前者地址存入后者 第2个参数
  401467:	48 8d 46 14          	lea    0x14(%rsi),%rax !# 第6个参数
  40146b:	48 89 44 24 08       	mov    %rax,0x8(%rsp) # 栈上传参的方式
  401470:	48 8d 46 10          	lea    0x10(%rsi),%rax !# 第5个参数
  401474:	48 89 04 24          	mov    %rax,(%rsp)
  401478:	4c 8d 4e 0c          	lea    0xc(%rsi),%r9 !# 第4个参数
  40147c:	4c 8d 46 08          	lea    0x8(%rsi),%r8 #这里即将六位数字存到栈中 3
  401480:	be c3 25 40 00       	mov    $0x4025c3,%esi # scanf的格式读入
  401485:	b8 00 00 00 00       	mov    $0x0,%eax
  40148a:	e8 61 f7 ff ff       	call   400bf0 <__isoc99_sscanf@plt>
  40148f:	83 f8 05             	cmp    $0x5,%eax # 判断scanf的返回值，即确认是否超过五个数字
